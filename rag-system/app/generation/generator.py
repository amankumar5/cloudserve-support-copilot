...
class GroundedGenerator:
    ...
    def generate_answer(
        ...
        if self.provider == "gemini" and self.gemini_key:
            answer_text = self._call_gemini(system_prompt, user_prompt, chat_history)
        elif self.provider == "openai" and self.openai_key:
            answer_text = self._call_openai(system_prompt, user_prompt, chat_history)
        else:
    ...
    def _call_openai(self, system_prompt: str, user_prompt: str, chat_history: Optional[List[ChatMessage]]) -> str:
        try:
            ...
            response = client.chat.completions.create(model=settings.OPENAI_MODEL, messages=messages, temperature=0.0)
